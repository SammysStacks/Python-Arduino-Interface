...
class eogProtocol(_globalProtocol.globalProtocol):
    ...
    def __init__(self, numPointsPerBatch = 3000, moveDataFinger = 10, numChannels = 2, plottingClass = None, readData = None):
        ...
        self.cutOffFreq = [.1, 15]        # Optimal LPF Cutoff in Literatrue is 6-8 or 20 Hz (Max 35 or 50); I Found 20 Hz was the Best, but can go to 15 if noisy (small amplitude cutoff)
    ...
    def initPlotPeaks(self): 
        ...
        axes = self.plottingClass.axes['eog'][0]
        ...
        for channelIndex in range(self.numChannels):
            ...
            # Generate Plot
            self.bioelectricDataPlots.append(self.bioelectricPlotAxes[channelIndex].plot([], [], '-', c="tab:blue", linewidth=1, alpha = 0.65)[0])
            
            # Set Figure Limits
            self.bioelectricPlotAxes[channelIndex].set_ylim(yLimLow, yLimHigh)
            # Label Axis + Add Title
            self.bioelectricPlotAxes[channelIndex].set_ylabel("EOG Signal (Volts)", fontsize=13, labelpad = 10)
        ...
        for channelIndex in range(self.numChannels):
            ...
            # Plot Flitered Peaks
            self.filteredBioelectricDataPlots.append(self.filteredBioelectricPlotAxes[channelIndex].plot([], [], '-', c="tab:blue", linewidth=1, alpha = 0.65)[0])
    ...
    def analyzeData(self, dataFinger, calibrateModel = False):     
        ...
        for channelIndex in range(self.numChannels):
            ...
            if channelIndex == 0 and self.collectFeatures:
                ...
                unAnalyzedDataPointer_NoCulling = max(0, self.lastAnalyzedDataInd - startFilterPointer)
                ...
                self.findBlinks(unAnalyzedTimes, unAnalyzedData)
            # --------------------------------------------------------------- #
            ...
            if self.plotStreamedData and not calibrateModel:
                ...
                # self.trailingAveragePlots[channelIndex].set_data(filteredTime, self.trailingAverageData[channelIndex][-len(timePoints):])
...
# ------------------------- Signal Analysis --------------------------------- #
    ...
    def findBlinks(self, xData, yData, debugBlinkDetection = False):
        ...
        for peakInd in peakIndices:
            ...
            if peakTimePoint <= self.data[0][self.lastAnalyzedDataInd]:
                continue
            
            # ------------------ Find the Peak's Baselines ------------------ #
            # Calculate the baseline of the peak.
            leftBaselineIndex = self.universalMethods.findNearbyMinimum(yData, peakInd, binarySearchWindow = -max(1, int(self.samplingFreq*0.002)), maxPointsSearch = max(1, int(self.samplingFreq*0.5)))
            rightBaselineIndex = self.universalMethods.findNearbyMinimum(yData, peakInd, binarySearchWindow = max(1, int(self.samplingFreq*0.002)), maxPointsSearch = max(1, int(self.samplingFreq*0.5)))
            ...
            self.lastAnalyzedDataInd = self.findStartFeatureWindow(self.lastAnalyzedDataInd, peakTimePoint, 0)
            ...
            leftBaselineIndex -= max(leftBaselineIndex, int(self.samplingFreq*0.05))
            ...
            self.readData.averageFeatures([peakTimePoint], [newFeatures], self.featureTimes, self.rawFeatures, self.compiledFeatures, self.featureAverageWindow)
            # --------------------------------------------------------------- #
        ...
        if len(xData) != 0:
            self.lastAnalyzedDataInd = self.findStartFeatureWindow(self.lastAnalyzedDataInd, xData[-1], 10)
    ...
    def extractFeatures(self, xData, yData, peakInd, debugBlinkDetection = True):
        ...
        # Cull off excess part of the peak
        # yData = yData[]
        # xData = xData[]
        ...
        accel23_AmpRatio = yData[peakBoundaries[1]]/yData[accelInds[2]]
        # ------------------------------------------------------------------- #
        ...
        openingSlope_MaxAccel2 = firstDeriv[peakBoundaries[1]]
        ...
        vel0_Accel2_Ratio = firstDeriv[velInds[0]]/secondDeriv[accelInds[2]]  
        ...
        if startBlinkInd == accelInds[0]:
            ...
            indexOptions = [startBlinkInd, accelInds[0]]
            portion1Integral = (min(indexOptions) == startBlinkInd) * scipy.integrate.simpson(yData[min(indexOptions):max(indexOptions)], xData[min(indexOptions):max(indexOptions)])
        ...
        portion8Integral = 0
        if peakBoundaries[1] < endBlinkInd:
            portion8Integral = scipy.integrate.simpson(yData[peakBoundaries[1]:endBlinkInd], xData[peakBoundaries[1]:endBlinkInd])
        ...
        peakKurtosis = scipy.stats.kurtosis(fullBlink, fisher=True, bias = False)
        ...
        peakFeatures.extend([peakHeight, tentDeviationX, tentDeviationY, tentDeviationRatio])
        ...
        peakFeatures.extend([accel01_AmpRatio, accel0_Vel0_AmpRatio, velAmpRatio, accel2_Vel1_AmpRatio, accel23_AmpRatio])
        ...
        peakFeatures.extend([accel0_Vel0_Ratio, accel0_Accel1_Ratio, accel0_Peak_Ratio, accel0_Vel1_Ratio, accel0_Accel2_Ratio])
        peakFeatures.extend([vel0_Accel1_Ratio, vel0_Peak_Ratio, velRatio, vel0_Accel2_Ratio])
        peakFeatures.extend([accel1_Peak_Ratio, accel1_Vel1_Ratio, accel1_Accel2_Ratio])
        peakFeatures.extend([vel1_Peak_Ratio, vel1_Accel2_Ratio, accelRatio2])
